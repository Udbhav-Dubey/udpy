...
# Detection variables
phone_detection_count = 0  # Total discrete phone appearances
N = 3  # Threshold: total phone appearances to trigger disqualification
PHONE_CLASS_ID = 67  # COCO class ID for 'cell phone'

# State tracking
phone_currently_detected = False
frames_without_phone = 0
COOLDOWN_FRAMES = 15  # Frames to wait before counting a new detection
...
while True:
    ...
    if phone_detected_this_frame:
        if not phone_currently_detected and frames_without_phone >= COOLDOWN_FRAMES:
            ...
            # Phone detected but still in cooldown period
            frames_without_phone = 0
    else:
        # No phone detected
    ...
    if phone_detection_count >= N:
        ...
        cv2.imshow("Phone Detection", annotated_frame)
        cv2.waitKey(3000)  # Show disqualification message for 3 seconds
        break
